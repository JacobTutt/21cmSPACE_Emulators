...
def load_t21_package(path: str | Path) -> dict[str, Any]:
    """Load a saved T21 package and validate that it contains metadata."""
    package = load(path)
...
def predict_t21(
    ...
    """Predict T21 on a chosen redshift grid for one or more parameter sets."""
    package = (
    ...
    axis_features = apply_transform(z, spec.axes[0].transform)[:, None]
...
def describe_t21_package(path: str | Path) -> dict[str, Any]:
    """Return a small human-readable summary of a saved T21 package."""
